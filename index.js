...
createMenuItem("egg", 3, "breakfast");
createMenuItem("pizza", 5, "lunch");
createMenuItem("burger", 7, "dinner");
...
*/
reviews.push({ name: "new", rating: 5.5, feedback: "new" })]
console.log(reviews);
...
function getReviewByIndex(list, idx) {
  const { name, rating, feedback} = list[idx]
...
function getLastReview(list) {
  const { name, rating, feedback } = list[-1] 
  return `${name} gave the restaurant a ${rating} star review, and their feedback was: ${feedback}`
}
